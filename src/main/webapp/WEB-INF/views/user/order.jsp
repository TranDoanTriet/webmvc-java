			...
			<div class="order-item">
				<div class="row">
					...
					<div class="col-1">
						<c:choose>
					        <c:when test="${billDataByUser.status}">
					            <a class="btn btn-success disabled-link" href="<c:url value="/user/received/${billDataByUser.idbill}"/>">Received</a>
					        </c:when>
					        <c:otherwise>
					            <a class="btn btn-success" href="<c:url value="/user/received/${billDataByUser.idbill}"/>">Received</a>
					        </c:otherwise>
					    </c:choose>
					</div>
					<div class="col-1">
						<c:choose>
					        <c:when test="${billDataByUser.status}">
					            <a class="btn btn-danger disabled-link" href="<c:url value="/user/order/${billDataByUser.idbill}"/>">Cancel</a>
					        </c:when>
					        <c:otherwise>
					            <a class="btn btn-danger" href="<c:url value="/user/order/${billDataByUser.idbill}"/>">Cancel</a>
					        </c:otherwise>
					    </c:choose>
					</div>
				</div>
